...
For distribution, I built a proper per-user Windows installer with Inno Setup. It bundles the trimmed JavaFX runtime, adds shortcuts, registers an uninstaller, and does not require administrator rights or a separate Java installation.
...
On Windows, download and run `Install-DaybreakProtocol.exe`. It installs the bundled JavaFX runtime, Start Menu shortcut, optional Desktop shortcut, and Windows uninstaller. A portable `DaybreakProtocol.exe` build is also included. Controls are `1` to `4` for answers, `H` for a hint, and `Enter` to continue. Mouse controls are also supported.
